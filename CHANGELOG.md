...
- `tk show` lineage block — compact summary at the top of show output listing the ticket's `parent`, `children`, `plan`, and `epic` (any of those that have values). Each field appears as `key: value`; missing fields are omitted; tickets with no lineage at all emit nothing extra. Improves navigation across decomposed parent/child chains. The existing detailed `## Children`, `## Blockers`, `## Blocking`, and `## Linked` sections remain at the bottom for full status/title info.
